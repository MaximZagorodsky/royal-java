...
public class CategoryUtil {
    //TODO если компании будут меняться то необходимо придумать и реализовать общий механизм
//    установки и получения тарифов для всех компаний, текущее решение - "временное"
    ...
    private static int getCategoryBySimpleDay(DateTime date) {
        int dayOfWeek;
        System.out.println("dauOfweek:"+date.getDayOfWeek());
    ...
    public static int getCategoryByDateAndCompanyNameAndType(DateTime date, String companyName, int type) {
        ...
        switch (company) {
            case ROYAL_MOVING:
                category = getRoyalMovingPriceCategoryByDay(date);
                break;
            case LIONS_MOVING:
                category = type;
                break;
